...
<title>demo: Chokidar 1.7.0 (May 8, 2017)</title>
...
<div class="title">Chokidar 1.7.0 (May 8, 2017) </div>  </div>
</div><!--header-->
<div class="contents">
<div class="textblock"><ul>
<li>Add <code>disableGlobbing</code> option</li>
<li>Add ability to force interval value by setting CHOKIDAR_INTERVAL env variable</li>
<li>Fix issue with <code>.close()</code> being called before <code>ready</code></li>
</ul>
<h1>Chokidar 1.6.0 (Jun 22, 2016)</h1>
<ul>
<li>Added ability for force <code>usePolling</code> mode by setting <code>CHOKIDAR_USEPOLLING</code> env variable</li>
</ul>
<h1>Chokidar 1.5.2 (Jun 7, 2016)</h1>
<ul>
...
<h1>Chokidar 1.5.1 (May 20, 2016)</h1>
<ul>
<li>To help prevent exhaustion of FSEvents system limitations, consolidate watch instances to the common parent upon detection of separate watch instances on many siblings</li>
</ul>
<h1>Chokidar 1.5.0 (May 10, 2016)</h1>
<ul>
...
<h1>Chokidar 1.3.0 (Nov 18, 2015)</h1>
<ul>
...
<h1>Chokidar 1.2.0 (Oct 1, 2015)</h1>
<ul>
...
<h1>Chokidar 0.12.4 (Dec 14, 2014)</h1>
<ul>
...
<h1>Chokidar 0.10.5 (Nov 6, 2014)</h1>
<ul>
...
<h1>Chokidar 0.10.4 (Nov 5, 2014)</h1>
<ul>
...
<h1>Chokidar 0.10.3 (Oct 28, 2014)</h1>
<ul>
...
<h1>Chokidar 0.9.0 (Sep 25, 2014)</h1>
<ul>
...
<h1>Chokidar 0.8.2 (Mar 26, 2014)</h1>
<ul>
<li>Fixed npm issues related to fsevents dep.</li>
<li>Updated fsevents to 0.2.</li>
</ul>
<h1>Chokidar 0.8.1 (Dec 16, 2013)</h1>
<ul>
<li>Optional deps are now truly optional on windows and linux.</li>
<li>Rewritten in JS, again.</li>
<li>Fixed some FSEvents-related bugs.</li>
</ul>
<h1>Chokidar 0.8.0 (Nov 29, 2013)</h1>
<ul>
<li>Added ultra-fast low-CPU OS X file watching with FSEvents. It is enabled by default.</li>
<li>Added <code>addDir</code> and <code>unlinkDir</code> events.</li>
<li>Polling is now disabled by default on all platforms.</li>
</ul>
<h1>Chokidar 0.7.1 (Nov 18, 2013)</h1>
<ul>
<li><code>Watcher::close</code> now also removes all event listeners.</li>
</ul>
<h1>Chokidar 0.7.0 (Oct 22, 2013)</h1>
<ul>
<li>When <code>options.ignored</code> is two-argument function, it will also be called after stating the FS, with <code>stats</code> argument.</li>
<li><code>unlink</code> is no longer emitted on directories.</li>
</ul>
<h1>Chokidar 0.6.3 (Aug 12, 2013)</h1>
<ul>
<li>Added <code>usePolling</code> option (default: <code>true</code>). When <code>false</code>, chokidar will use <code>fs.watch</code> as backend. <code>fs.watch</code> is much faster, but not like super reliable.</li>
</ul>
<h1>Chokidar 0.6.2 (Mar 19, 2013)</h1>
<ul>
<li>Fixed watching initially empty directories with <code>ignoreInitial</code> option.</li>
</ul>
<h1>Chokidar 0.6.1 (Mar 19, 2013)</h1>
<ul>
<li>Added node.js 0.10 support.</li>
</ul>
<h1>Chokidar 0.6.0 (Mar 10, 2013)</h1>
<ul>
<li>File attributes (stat()) are now passed to <code>add</code> and <code>change</code> events as second arguments.</li>
<li>Changed default polling interval for binary files to 300ms.</li>
</ul>
<h1>Chokidar 0.5.3 (Jan 13, 2013)</h1>
<ul>
<li>Removed emitting of <code>change</code> events before <code>unlink</code>.</li>
</ul>
<h1>Chokidar 0.5.2 (Jan 13, 2013)</h1>
<ul>
<li>Removed postinstall script to prevent various npm bugs.</li>
</ul>
<h1>Chokidar 0.5.1 (Jan 6, 2013)</h1>
<ul>
<li>When starting to watch non-existing paths, chokidar will no longer throw ENOENT error.</li>
<li>Fixed bug with absolute path.</li>
</ul>
<h1>Chokidar 0.5.0 (Dec 9, 2012)</h1>
<ul>
...
<li><code>interval</code> and <code>binaryInterval</code> that allow to change default fs polling intervals.</li>
</ul>
</li>
</ul>
<h1>Chokidar 0.4.0 (Jul 26, 2012)</h1>
<ul>
<li>Added <code>all</code> event that receives two args (event name and path) that combines <code>add</code>, <code>change</code> and <code>unlink</code> events.</li>
<li>Switched to <code>fs.watchFile</code> on node.js 0.8 on windows.</li>
<li>Files are now correctly unwatched after unlink.</li>
</ul>
<h1>Chokidar 0.3.0 (Jun 24, 2012)</h1>
<ul>
<li><code>unlink</code> event are no longer emitted for directories, for consistency with <code>add</code>.</li>
</ul>
<h1>Chokidar 0.2.6 (Jun 8, 2012)</h1>
<ul>
<li>Prevented creating of duplicate 'add' events.</li>
</ul>
<h1>Chokidar 0.2.5 (Jun 8, 2012)</h1>
<ul>
<li>Fixed a bug when new files in new directories hadn't been added.</li>
</ul>
<h1>Chokidar 0.2.4 (Jun 7, 2012)</h1>
<ul>
<li>Fixed a bug when unlinked files emitted events after unlink.</li>
</ul>
<h1>Chokidar 0.2.3 (May 12, 2012)</h1>
<ul>
<li>Fixed watching of files on windows.</li>
</ul>
<h1>Chokidar 0.2.2 (May 4, 2012)</h1>
<ul>
<li>Fixed watcher signature.</li>
</ul>
<h1>Chokidar 0.2.1 (May 4, 2012)</h1>
<ul>
<li>Fixed invalid API bug when using <code>watch()</code>.</li>
</ul>
<h1>Chokidar 0.2.0 (May 4, 2012)</h1>
<ul>
<li>Rewritten in js.</li>
</ul>
<h1>Chokidar 0.1.1 (Apr 26, 2012)</h1>
<ul>
<li>Changed api to <code>chokidar.watch()</code>.</li>
<li>Fixed compilation on windows.</li>
</ul>
<h1>Chokidar 0.1.0 (Apr 20, 2012)</h1>
<ul>
