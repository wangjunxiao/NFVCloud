...
<title>demo: Extension pipelining</title>
...
<div class="title">Extension pipelining </div>  </div>
</div><!--header-->
<div class="contents">
<div class="textblock"><p><code>websocket-extensions</code> models the extension negotiation and processing pipeline of the WebSocket protocol. Between the driver parsing messages from the TCP stream and handing those messages off to the application, there may exist a stack of extensions that transform the message somehow.</p>
...
+-------------+  in   +---+     +---+     +---+       +--------+
                      ...
                      \                       /
                       +----------o----------+
                                  |
                               sessions
</pre><p>For outgoing messages, the driver receives the result of </p><pre class="fragment">    C.outgoing(B.outgoing(A.outgoing(message)))

or, [A, B, C].reduce(((m, ext) =&gt; ext.outgoing(m)), message)
</pre><p>For incoming messages, the application receives the result of </p><pre class="fragment">    A.incoming(B.incoming(C.incoming(message)))

or, [C, B, A].reduce(((m, ext) =&gt; ext.incoming(m)), message)
</pre><p>A session is of the following type, to borrow notation from pseudo-Haskell: </p><pre class="fragment">type Session = {
  incoming :: Message -&gt; Message
  outgoing :: Message -&gt; Message
  close    :: () -&gt; ()
}
</pre><p>(That <code>() -&gt; ()</code> syntax is intended to mean that <code>close()</code> is a nullary void method; I apologise to any Haskell readers for not using the right monad.)</p>
...
<p>However because this is JavaScript, the <code>incoming()</code> and <code>outgoing()</code> methods may be asynchronous (indeed, <code>permessage-deflate</code> is based on <code>zlib</code>, whose API is stream-based). So their interface is strictly: </p><pre class="fragment">type Session = {
...
</pre><p>Here's the extension:</p>
<div class="fragment"><div class="line">var zlib = require(&#39;zlib&#39;);</div><div class="line"></div><div class="line">var deflate = {</div><div class="line">  outgoing: function(message, callback) {</div><div class="line">    zlib.deflateRaw(message.data, function(error, result) {</div><div class="line">      message.rsv1 = true;</div><div class="line">      message.data = result;</div><div class="line">      callback(error, message);</div><div class="line">    });</div><div class="line">  },</div><div class="line"></div><div class="line">  incoming: function(message, callback) {</div><div class="line">    // decompress inbound messages (elided)</div><div class="line">  },</div><div class="line"></div><div class="line">  close: function() {</div><div class="line">    // no state to clean up</div><div class="line">  }</div><div class="line">};</div></div><!-- fragment --><p>We can call it with a large message followed by a small one, and the small one will be returned first:</p>
<div class="fragment"><div class="line">var crypto = require(&#39;crypto&#39;),</div><div class="line">    large  = crypto.randomBytes(1 &lt;&lt; 14),</div><div class="line">    small  = new Buffer(&#39;hi&#39;);</div><div class="line"></div><div class="line">deflate.outgoing({data: large}, function() {</div><div class="line">  console.log(1, &#39;large&#39;);</div><div class="line">});</div><div class="line"></div><div class="line">deflate.outgoing({data: small}, function() {</div><div class="line">  console.log(2, &#39;small&#39;);</div><div class="line">});</div><div class="line"></div><div class="line">/* prints:  2 &#39;small&#39;</div><div class="line">            1 &#39;large&#39; */</div></div><!-- fragment --><p>So a session that processes messages asynchronously may fail to preserve message ordering.</p>
...
<p>For example, it is entirely possible to take the simple <code>deflate</code> extension above and wrap its <code>incoming()</code> and <code>outgoing()</code> methods in two <code>Transform</code> streams, producing this type: </p><pre class="fragment">type Session = {
  incoming :: TransformStream
  outtoing :: TransformStream
  close    :: () -&gt; ()
}
</pre><p>The <code>Transform</code> class makes it easy to wrap an async function such that message order is preserved:</p>
<div class="fragment"><div class="line">var stream  = require(&#39;stream&#39;),</div><div class="line">    session = new stream.Transform({objectMode: true});</div><div class="line"></div><div class="line">session._transform = function(message, _, callback) {</div><div class="line">  var self = this;</div><div class="line">  deflate.outgoing(message, function(error, result) {</div><div class="line">    self.push(result);</div><div class="line">    callback();</div><div class="line">  });</div><div class="line">};</div></div><!-- fragment --><p>However, this has a negative impact on throughput: it works by deferring <code>callback()</code> until the async function has 'returned', which blocks <code>Transform</code> from passing further input into the <code>_transform()</code> method until the current message is dealt with completely. This would prevent sessions from processing messages concurrently, and would unnecessarily reduce the throughput of stateless extensions.</p>
...
<pre class="fragment">        +--------+
      ...
      +------------+      +-----o----+
          ...
          +----------+    +----------+    +----------+
</pre><p>A driver using this framework holds an instance of the <code>Extensions</code> class, which it uses to register extension plugins, negotiate headers and transform messages. The <code>Extensions</code> instance itself holds a <code>Pipeline</code>, which contains an array of <code>Cell</code> objects, each of which wraps one of the sessions.</p>
...
<p>Internally, a <code>Cell</code> contains two <code>Functor</code> objects. A <code>Functor</code> wraps an async function and makes sure its output messages maintain the order of its input messages. This name is due to <a href="https://github.com/fronx"></a>, on the basis that, by preserving message order, the abstraction preserves the <em>mapping</em> between input and output messages. To use our simple <code>deflate</code> extension from above:</p>
<div class="fragment"><div class="line">var functor = new Functor(deflate, &#39;outgoing&#39;);</div><div class="line"></div><div class="line">functor.call({data: large}, function() {</div><div class="line">  console.log(1, &#39;large&#39;);</div><div class="line">});</div><div class="line"></div><div class="line">functor.call({data: small}, function() {</div><div class="line">  console.log(2, &#39;small&#39;);</div><div class="line">});</div><div class="line"></div><div class="line">/*  -&gt;  1 &#39;large&#39;</div><div class="line">        2 &#39;small&#39; */</div></div><!-- fragment --><p>A <code>Cell</code> contains two of these, one for each direction:</p>
<pre class="fragment">                        +-----------------------+
                  +----&gt;| Functor [A, incoming] |
+----------+      |     +-----------------------+
| Cell [A] o------+
+----------+      |     +-----------------------+
                  +----&gt;| Functor [A, outgoing] |
                        +-----------------------+
</pre><p>This satisfies the message transformation requirements: the <code>Pipeline</code> simply loops over the cells in the appropriate direction to transform each message. Because each <code>Cell</code> will preserve message order, we can pass a message to the next <code>Cell</code> in line as soon as the current <code>Cell</code> returns it. This gives each <code>Cell</code> all the messages in order while maximising throughput.</p>
<h3>Session closing</h3>
...
<pre class="fragment">                          +----------+
          ...
          +----------+    +----------+    +----------+
             in: 0           in: 0           in: 0
            out: 0          out: 0          out: 0
</pre><p>When a message <em>m1</em> enters the pipeline, say in the <code>outgoing</code> direction, we increment the <code>pending.out</code> counter on all cells immediately.</p>
<pre class="fragment">                          +----------+
                    m1 =&gt; | Pipeline |
                          +-----o----+
                                |
          ...
          +----------+    +----------+    +----------+
             in: 0           in: 0           in: 0
            out: 1          out: 1          out: 1
</pre><p><em>m1</em> is handed off to <code>A</code>, meanwhile a second message <code>m2</code> arrives in the same direction. All <code>pending.out</code> counters are again incremented.</p>
<pre class="fragment">                          +----------+
                    m2 =&gt; | Pipeline |
                          +-----o----+
                                |
          ...
          +----------+    +----------+    +----------+
             in: 0           in: 0           in: 0
            out: 2          out: 2          out: 2
</pre><p>When the first cell's <code>A.outgoing</code> functor finishes processing <em>m1</em>, the first <code>pending.out</code> counter is decremented and <em>m1</em> is handed off to cell <code>B</code>.</p>
<pre class="fragment">                          +----------+
                          | Pipeline |
                          +-----o----+
                                |
          ...
          +----------+    +----------+    +----------+
             in: 0           in: 0           in: 0
            out: 1          out: 2          out: 2
</pre><p>As <code>B</code> finishes with <em>m1</em>, and as <code>A</code> finishes with <em>m2</em>, the <code>pending.out</code> counters continue to decrement.</p>
<pre class="fragment">                          +----------+
                          | Pipeline |
                          +-----o----+
                                |
          ...
          +----------+    +----------+    +----------+
             in: 0           in: 0           in: 0
            out: 0          out: 1          out: 2
</pre><p>Say <code>C</code> is a little slow, and begins processing <em>m2</em> while still processing <em>m1</em>. That's fine, the <code>Functor</code> mechanism will keep <em>m1</em> ahead of <em>m2</em> in the output.</p>
<pre class="fragment">                          +----------+
                          | Pipeline |
                          +-----o----+
                                |
          ...
          +----------+    +----------+    +----------+
             in: 0           in: 0           in: 0
            out: 0          out: 0          out: 2
</pre><p>Once all messages are dealt with, the counters return to <code>0</code>.</p>
<pre class="fragment">                          +----------+
                          | Pipeline |
                          +-----o----+
                                |
          ...
          +----------+    +----------+    +----------+
             in: 0           in: 0           in: 0
            out: 0          out: 0          out: 0
</pre><p>The same process applies in the <code>incoming</code> direction, the only difference being that messages are passed to <code>C</code> first.</p>
...
<p>The actual closing API at each level is slightly different: </p><pre class="fragment">type Session = {
  close :: () -&gt; ()
}
...
<pre class="fragment">                          +----------+
          ...
          +----------+    +----------+    +----------+
             in: 0           in: 0           in: 0
            out: 3          out: 3          out: 3
</pre><p>They pass through the cells successfully up to this point:</p>
<pre class="fragment">                          +----------+
                          | Pipeline |
                          +-----o----+
                                |
...
<pre class="fragment">                          +----------+
          ...
          +----------+    +----------+    +----------+
             in: 0           in: 0           in: 0
            out: 0          out: 0          out: 2
</pre><p>When a cell sees its session emit an error, or when it receives an error from upstream, it sets its pending count in the appropriate direction to equal the number of messages it is <em>currently</em> processing. It will not accept any messages after it sees the error, so this will allow the counter to reach zero.</p>
...
<p>Credit is due to <a href="https://github.com/mnowster"></a> for helping with the design and to <a href="https://github.com/fronx"></a> for helping name things. </p>
</div></div><!-- contents -->
<!-- start footer part -->
